...
const HeroBlog = () => {
  ...
  return (
    <div className='w-full justify-center md:w-[75vw] flex flex-wrap overflow-scroll gap-3 h-[83vh]' >
      {
       searchedBlogs?searchedBlogs.map((item,index)=>(
        <div onClick={() => {
            router.push(`blog/${item._id}`) } }
            key={index} >
          <BlogPost image={item.featuredImage} username={item.userData.username} logo={item.userData.profilePic} title={item.title}  />
        </div>
       )): blogLists.map((item, index) => (
          <div onClick={() => {
            router.push(`blog/${item._id}`)
          }} className=' h-fit w-fit ' key={index} >
            <BlogPost username={item?.userData?.username} logo={item.userData?.profilePic} image={item.featuredImage} title={item.title} />
          </div>
        ))
      }
    </div>
